...
function CadastroEvento() {
  ...
  // LOCAL DO EVENTO
  const [locationName, setLocationName] = useState('Trilha dos dedos');
  ...
  const [horarioInicioEvento, setHorarioInicioEvento] = useState('22:33')
  const [endDate, setEndDate] = useState('2022-12-01');
  const [horarioTerminoEvento, setHorarioTerminoEvento] = useState('22:33')
  ...
  const [horaInicioVendas, setHoraInicioVendas] = useState('22:33')
  const [endOfSales, setEndOfSales] = useState('2022-12-01');
  const [horaTerminoVendas, setHoraTerminoVendas] = useState('22:33')
...
const handleDrop = (e) => {
  e.preventDefault()
  setMap_description(e.dataTransfer.files)
}
      ...
      <div className='row'>
        ...
        <div className="col-12 mt-5">
          <form className="row g-3 " onSubmit={createEvent}>
            ...
            {/* <div className="col-12" onDrop={handleDrop} onDragOver={handleDragOver}>
              <input type="file" id='arquivo' name='arquivo' className='d-none' ref={inputRef} onChange={(e) => setfile(e.target.files)} />
              <label htmlFor="arquivo" className="btn add-map" onClick={() => inputRef.current.click()}><p>Arraste ou clique para adicionar seu mapa</p></label>
            </div>
            {file && (
              <div>
                <ul>
                  {Array.from(file).map((file, id) => <li key={id}>{file.name}</li>)}
                </ul>
              </div>
            )
            } */}
